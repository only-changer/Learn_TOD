...
class Model:
    ...
    def train(self, convert_type, flow_type):
        ...
        for o in range(10):
            ...
            OD_list = {"1,3,4": np.random.randint(1, 20, size=(12, 1)),
                       "3,4,2": np.random.randint(1, 20, size=(12, 1)),
                       "4,2,1": np.random.randint(1, 20, size=(12, 1))}
            otd = []
            for i in range(len(list(OD_list.keys()))):
                otd.append(np.ones((self.n_time_interval, 1)) * np.average(test_otd[i]))
