...
class Model:
    ...
    def build_network(self, OD, speed):
        ...
        with tf.variable_scope("speed2od"):
            tensor_otd = tf.reshape(list_OD, (-1, self.n_od ))
            speed = tf.reshape(speed, (-1, self.n_road ))
            hid = tf.layers.dropout(tf.nn.relu(tf.layers.dense(speed, self.units_size)), rate=0.3)
            out = tf.nn.sigmoid(tf.layers.dense(hid, self.n_od )) * 100
    ...
    def train(self, convert_type, flow_type):
        ...
        for i in range(len(speeds)):
            for j in range(self.n_time_interval):
                speeds[i][0][0][j] = speeds[i][0][0][j] / 10
        for i in range(len(test_speeds)):
            for j in range(self.n_time_interval):
                test_speeds[i][0][0][j] = test_speeds[i][0][0][j] / 10
        if convert_type == 'cityflow':
            simulator = Simulator()
            speeds = simulator.v2s(volumes)
            test_speeds = simulator.v2s(test_volumes)
        input = np.reshape(input, (-1,  self.n_time_interval * self.n_od ))
        test_volumes = np.reshape(test_volumes, (-1, self.n_road, self.n_time_interval))
        speeds = np.reshape(speeds, (-1, self.n_time_interval, self.n_road ))
        test_input = np.reshape(test_input, (-1, self.n_od * self.n_time_interval))
        test_speeds = np.reshape(test_speeds, (-1,  self.n_time_interval , self.n_road))
        ...
        model = Sequential()
        model.add(LSTM(128, input_shape=(None, self.n_road), return_sequences=True))
        model.add(LSTM(128))
        model.add(Dense(128))
        model.add(Dense(self.n_od * self.n_time_interval))
        model.compile('adam', 'mse')
        model.fit(speeds, input, batch_size=128, epochs=300, verbose=2, validation_data=[test_speeds,test_input])

        loss_od = 0.0
        loss_volume = 0.0
        loss_speed = 0.0
        loss_gls[convert_type][flow_type] = [convert_type, flow_type]
        out_otd = model.predict(test_speeds)
        out_otd = np.reshape(out_otd, (-1, self.n_od, self.n_time_interval))
        for o in range(100):
            otd = out_otd[o]
            ...
            test_otd = np.reshape(test_otd, (self.n_od, self.n_time_interval))
            ...
            test_speed = np.reshape(test_speed, (-1, self.n_road, self.n_time_interval))
            loss_od += rmse(otd, test_otd)
            loss_volume += rmse(volume, test_volume)
            loss_speed += rmse(speed, test_speed)
            print("real_od", test_otd)
            print("pred_od", otd)
            print("real_volume", test_volume)
            print("pred_volume", volume)
            print("real_speed", test_speed)
            print("pred_speed", speed)
            if o == 0:
                with open('lstm_5.7_case_new.csv', 'w') as file:
                    write = csv.writer(file)
                    for i in range(self.n_od):
                        write.writerow(otd[i])
        loss_od = loss_od / 100
...
if __name__ == "__main__":
    ...
    for c in convert:
        ...
        for f in flow:
            ...
            model = Model()
            model.train(c,f)
    with open('lstm_5.7_new.csv', 'w') as f:
        writer = csv.writer(f)
        for c in convert:
            for fl in flow:
