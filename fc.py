...
class Model:
    ...
    def build_network(self, OD, speed):
        ...
        with tf.variable_scope("speed2od"):
            ...
            # hid1 = tf.layers.dropout(tf.nn.sigmoid(tf.layers.dense(speed, self.units_size)), rate=0.3)
            # hid2 = tf.layers.dropout(tf.nn.sigmoid(tf.layers.dense(hid1, self.units_size)), rate=0.3)
            # out = tf.nn.sigmoid(tf.layers.dense(hid2, self.n_od * self.n_time_interval)) * 100
            out = []
            for road_ind in range(self.n_od):
                road_speed = tf.gather(speed, road_ind, axis=1)
                hid = tf.layers.dropout(tf.nn.sigmoid(tf.layers.dense(road_speed, self.units_size)), rate=0.3)
                hid = tf.layers.dropout(tf.nn.relu(tf.layers.dense(hid, self.units_size)), rate=0.3)
                out.append(tf.reshape(tf.nn.relu(tf.layers.dense(hid, self.n_time_interval)),
    ...
    def train(self, convert_type, flow_type):
        ...
        with tf.Session() as sess:
            ...
            for epo in range(self.epoch):
                step += 1
                sess.run(optimizer,
                         feed_dict={otd_input: input, speed_label: speeds})
                train_loss = sess.run(loss,
                                      feed_dict={otd_input: input, speed_label: speeds})
                test_loss = sess.run(loss,
                                     feed_dict={otd_input: test_input, speed_label: test_speeds})
                print('step', step, ' train_loss', train_loss, '    test_loss', test_loss)
                if test_loss < min_loss:
                    min_loss = test_loss
                    min_loss_step = step
                if step - min_loss_step > self.early_stop:
                    break
            out_otd = sess.run(out,
                                 feed_dict={otd_input: test_input, speed_label: test_speeds})
...
if __name__ == "__main__":
    ...
    for c in convert:
        ...
        for f in flow:
            ...
            model = Model()
            model.train(c,f)
    with open('gls_5.7_old.csv', 'w') as f:
        writer = csv.writer(f)
        for c in convert:
            for fl in flow:
